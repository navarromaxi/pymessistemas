(() => {
  ...
  function render() {
    ...
    const days = dates.map((key) => `<button type="button" data-action="day" data-date="${key}" class="${key === selectedDate ? "is-active" : ""}"><span>${day.format(new Date(grouped[key][0]))}</span><b>${grouped[key].length} horarios</b></button>`).join("");
